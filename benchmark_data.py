...
def torch_dataloader(paths, world_size):
        ...
        num /= repeated
        total_size /= repeated
        training_time = (time.time() - start)/repeated
        print(f"Training time for {num} images of total {total_size} bytes pytorch: {training_time:.2f} seconds")
        throuput0 = num/training_time
        throuput1 = total_size/training_time
        print(f"files per second {throuput0}, filesize per second {throuput1}")
